...
public:
  void merge_envelopes(Minimization_diagram_2& result1,
    ...
    for (auto ei = result.edges_begin(); ei != result.edges_end(); ++ei) {
      ...
      // are continuous
      if (!get_aux_is_set(hh, 0) || !get_aux_is_set(hh, 1)) continue;
...
protected:
  ...
  bool is_equal_data(const InputIterator& begin1, const InputIterator& end1,
  ...
  bool can_remove_edge_target(Halfedge_handle h) {
    ...
    /* if (v->get_is_fake() && !v->is_decision_set()) return true;
    ...
    if (h->get_decision() != v->get_decision()) return false;
  ...
  bool can_remove_isolated_vertex(Vertex_handle vh) {
    ...
    // the envelope differs too.
    if (vh->get_decision() != f->get_decision()) return false;
  ...
  bool aux_has_no_data(FeatureHandle fh, unsigned int id) {
    ...
    // this is why we start with a check for a face, then halfedge
    // and last vertex
    if (assign(f, o)) return f->has_no_data();
    else if (assign(h, o)) return h->has_no_data();
    else {
      CGAL_assertion_code(bool b =)
      assign(v, o);
      CGAL_assertion(b);
...
protected:
  ...
  bool is_equal_data(Object o,
                     Envelope_data_iterator begin, Envelope_data_iterator end) {
    CGAL_assertion(!o.is_empty());
    Vertex_handle v;
    Halfedge_handle h;
    Face_handle f;

    if (assign(v, o)) return v->is_equal_data(begin, end);
    else if (assign(h, o)) return h->is_equal_data(begin, end);
    else {
  ...
  bool has_equal_data(Object o,
    ...
    if (assign(v, o)) return v->has_equal_data(begin, end);
    else if (assign(h, o)) return h->has_equal_data(begin, end);
    else {
      CGAL_assertion(assign(f, o));
      assign(f, o);
