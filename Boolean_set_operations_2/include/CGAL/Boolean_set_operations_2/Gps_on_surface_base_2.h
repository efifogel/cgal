...
#include <CGAL/Boolean_set_operations_2/Do_polygon_intersect_visitor.h>
#include <CGAL/Boolean_set_operations_2/Do_polygon_intersect_surface_sweep_2.h>
...
/*! `General_polygon_set_on_surface_2` class is the base class for
 * `General_polygon_set_on_surface_2` and receives extra template parameter
 * which allows different validation policies. If you do not want validation
 * then use the default validation policy. A different validation policy example
 * can be found in General_polygon_set_on_surface_2.
 */
template <typename Traits_, typename TopTraits_,
          typename ValidationPolicy = Boolean_set_operation_2_internal::NoValidationPolicy>
class Gps_on_surface_base_2 {
public:
  using Traits_2 = Traits_;
  using Topology_traits = TopTraits_;
  using Polygon_2 = typename Traits_2::Polygon_2;
  using Polygon_with_holes_2 = typename Traits_2::Polygon_with_holes_2;
  using Arrangement_on_surface_2 = CGAL::Arrangement_on_surface_2<Traits_2, Topology_traits>;
  using Size = typename Arrangement_on_surface_2::Size;

private:
  using Aos_2 = Arrangement_on_surface_2;

  using Self = Gps_on_surface_base_2<    Traits_2, Topology_traits, ValidationPolicy>;
  using Point_2 = typename Traits_2::Point_2;
  using X_monotone_curve_2 = typename Traits_2::X_monotone_curve_2;

  using GP_Holes_const_iterator = typename Polygon_with_holes_2::Hole_const_iterator;
  using Curve_const_iterator = typename Traits_2::Curve_const_iterator;
  using Compare_endpoints_xy_2 = typename Traits_2::Compare_endpoints_xy_2;
  using Construct_opposite_2 = typename Traits_2::Construct_opposite_2;

  using Face_const_iterator = typename Aos_2::Face_const_iterator;
  using Halfedge_const_iterator = typename Aos_2::Halfedge_const_iterator;
  using Vertex_const_iterator = typename Aos_2::Vertex_const_iterator;
  using Edge_const_iterator = typename Aos_2::Edge_const_iterator;
  using Outer_ccb_const_iterator = typename Aos_2::Outer_ccb_const_iterator;
  using Inner_ccb_const_iterator = typename Aos_2::Inner_ccb_const_iterator;
  using Ccb_halfedge_const_circulator = typename Aos_2::Ccb_halfedge_const_circulator;
  using Face_iterator = typename Aos_2::Face_iterator;
  using Halfedge_iterator = typename Aos_2::Halfedge_iterator;
  using Vertex_iterator = typename Aos_2::Vertex_iterator;
  using Edge_iterator = typename Aos_2::Edge_iterator;
  using Outer_ccb_iterator = typename Aos_2::Outer_ccb_iterator;
  using Inner_ccb_iterator = typename Aos_2::Inner_ccb_iterator;
  using Ccb_halfedge_circulator = typename Aos_2::Ccb_halfedge_circulator;
  using Face_handle = typename Aos_2::Face_handle;
  using Halfedge_handle = typename Aos_2::Halfedge_handle;
  using Vertex_handle = typename Aos_2::Vertex_handle;

  using Face_const_handle = typename Aos_2::Face_const_handle;
  using Halfedge_const_handle = typename Aos_2::Halfedge_const_handle;
...
public:
  ...
  Gps_on_surface_base_2& operator=(const Self& ps) {
    ...
    if (m_traits_owner) delete m_traits;
    delete m_arr;
    m_traits = new Traits_2(*(ps.m_traits));
...
public:
  ...
  void simplify(const Polygon_2& pgn, Polygon_with_holes_2& res) {
    using Simplifier = Gps_polygon_simplifier<Aos_2>;
    ...
    using OutputItr = Oneset_iterator<Polygon_with_holes_2>;
    OutputItr oi (res);
  ...
  void complement() { this->_complement(m_arr); }
  ...
  void fix_curves_direction() { _fix_curves_direction(*m_arr); }

  Size number_of_polygons_with_holes() const;

  // Traits_2& traits() { return *m_traits; }
  ...
  Oriented_side oriented_side(const Self& other) const {
    ...
    if (func.found_boundary_intersection()) return ON_ORIENTED_BOUNDARY;
    return ON_NEGATIVE_SIDE;
  }
...
protected:
  bool _is_valid(Aos_2& arr) {
    if (! CGAL::is_valid(arr)) return false;

    Compare_endpoints_xy_2 cmp_endpoints =
      m_traits->compare_endpoints_xy_2_object();

    for (auto eci = arr.edges_begin(); eci != arr.edges_end(); ++eci) {
      Halfedge_const_handle he = eci;
      if (he->face() == he->twin()->face()) return false;
      if (he->face()->contained() == he->twin()->face()->contained()) return false;
      ...
      if ((is_cont && !has_same_dir) || (!is_cont && has_same_dir)) return false;
    }
    return true;
  }
...
// Helper for Simple Polygons
  ...
  //! tests for intersection of a range of general polygons
...
protected:
  ...
  _halfedge(Halfedge_handle h) { return &(*h); }
  ...
  void set_flag_of_halfedges_of_final_argt(Halfedge_handle h, int flag) {
    ...
    do {
      h->set_flag(flag);
      h = h->next();
      while (is_redundant(h)) h = h->twin()->next();
    } while (start != h);
  }

  void _remove_redundant_edges(Aos_2* arr) {
    using Dcel = typename Aos_2::Dcel;
    ...
    using UF_faces = Union_find<Face*>;
    ...
    CGAL_assertion(std::distance(arr->unbounded_faces_begin(), arr->unbounded_faces_end()) == 1);
    ...
    for (auto itr = arr->halfedges_begin(); itr != arr->halfedges_end(); ++itr) {
      ...
      if (is_redundant(h)) {
        ...
        if(h->is_on_inner_ccb()) {
          ...
          if (h->twin()->flag() != NOT_VISITED || h->twin()->face()->id_not_set()) continue;
          //visit outer ccb of h in the final arrangement
          set_flag_of_halfedges_of_final_argt(h->twin(), ON_OUTER_CCB);
          std::size_t master_id = (*uf_faces.find(face_handles[h->twin()->face()->id()]))->id();
    ...
    do {
      ...
      for (auto it = uf_faces.begin(), it_end = uf_faces.end(); it != it_end; ++it) {
    ...
    for (auto vi = arr->vertices_begin(), vi_end = arr->vertices_end(); vi != vi_end; ++vi) {
      Vertex* v_ptr = &(*vi);
      Halfedge_handle h_start = vi->incident_halfedges();
      Halfedge_handle h = h_start;

      std::vector<Halfedge_handle> non_redundant_edges;
      bool found_no_redundant = true;
      do {
        if (! is_redundant(h)) non_redundant_edges.push_back(h);
        else found_no_redundant = false;
        h = h->next()->twin();
      } while (h != h_start);
      ...
      for (std::size_t i = 0; i < nb_edges; ++i) {
        Halfedge_handle h1 = non_redundant_edges[i];
        Halfedge_handle h2 = non_redundant_edges[i+1];
        if ( h1->next()->twin() != h2)
    ...
    std::vector<Face*> faces_to_remove;
    std::vector<typename Dcel::Outer_ccb*> outer_ccbs_to_remove;
    std::vector<typename Dcel::Inner_ccb*> inner_ccbs_to_remove;
    for (typename UF_faces::iterator it = uf_faces.begin(), it_end = uf_faces.end(); it != it_end; ++it) {
      typename UF_faces::handle master = uf_faces.find(it);
      //remove faces that are not the master of their set (but the unbounded face)
      if (master != it) {
      ...
      for (void* ptr : (*it)->_outer_ccbs())
        outer_ccbs_to_remove.push_back(static_cast<Halfedge*>(ptr)->outer_ccb());
      for (void* ptr : (*it)->_inner_ccbs())
        inner_ccbs_to_remove.push_back(static_cast<Halfedge*>(ptr)->inner_ccb());
    ...
    using Outer_ccb_and_face = std::pair<typename Dcel::Outer_ccb*, Face*>;
    using Inner_ccb_and_face = std::pair<typename Dcel::Inner_ccb*, Face*>;
    ...
    for (auto fit = arr->faces_begin(), end = arr->faces_end(); fit != end; ++fit) {
      fit->reset_id(); // reset the id that will no longer be used for this face
    }
  }
...
protected:
  ...
  bool _is_empty(const Polygon_with_holes_2& ) const { return (false); }

  bool _is_plane(const Polygon_2& ) const { return (false); }

  bool _is_plane(const Polygon_with_holes_2& pgn) const {
    // using Is_unbounded = typename Traits_2::Is_unbounded;
  ...
  void _join(const Polygon_& pgn) {
    ...
    if (_is_plane(pgn)) {
      ...
      for (auto fit = this->m_arr->faces_begin(); fit != this->m_arr->faces_end(); ++fit)
  ...
  void _join(const Self& other) {
    ...
    if (other.is_plane()) {
      ...
      // Even in an empty arrangement there can be several faces
      // (because of the topology traits).
      for (Face_iterator fit = this->m_arr->faces_begin(); fit != this->m_arr->faces_end(); ++fit)
        fit->set_contained(true);
      return;
    }
    if (this->is_empty()) {
